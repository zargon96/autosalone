...
//   const pendingTransitionX = useRef(null);
//   const pendingTransitionY = useRef(null);
...
//       pendingTransitionX.current = null;
...
//           if (pendingTransitionX.current) {
//             const { dir: d, onMidpoint: m } = pendingTransitionX.current;
//             runTransitionX(d, m);
//           }
//         },
//         onComplete: () => {
...
//         pendingTransitionX.current = { dir, onMidpoint };
//         return;
//       }
//       runTransitionX(dir, onMidpoint);
//     };
...
//       if (!groupRef.current) return;
//       isTransitioning.current = true;
//       pendingTransitionY.current = null;
...
//         onInterrupt: () => {
//           isTransitioning.current = false;
//           if (pendingTransitionY.current) {
//             const { dir: d, onMidpoint: m } = pendingTransitionY.current;
//             runTransitionY(d, m);
//           }
//         },
//         onComplete: () => {
...
//       if (isTransitioning.current) {
//         pendingTransitionY.current = { dir, onMidpoint };
//         return;
//       }
//       runTransitionY(dir, onMidpoint);
//     };
...
function useProgressiveLoader(entries, activeIndex) {
  const [gltfMap, setGltfMap] = useState({});
  ...
  const load = useCallback(
    (idx) => {
      ...
      loader.load(car.model, (gltf) => {
        ...
        if (car.scale) {
          if (typeof car.scale === "number") {
            scene.scale.setScalar(car.scale);
          } else {
            scene.scale.set(...car.scale);
          }
        }
        if (car.rotation) {
  ...
  const compiledRef = useRef(new Set());

  useEffect(() => {
    const toCompile = [activeIndex, activeIndex + 1].filter(
      (i) => gltfMap[i] && !compiledRef.current.has(i),
    );
    toCompile.forEach((i) => {
      compiledRef.current.add(i);
      const scene = gltfMap[i].scene;
      requestAnimationFrame(() => {
        try {
          gl.compile(scene, camera);
        } catch {}
...
const ModelsGroup = memo(function ModelsGroup({
  ...
  const activeIndex = useMemo(
    () => entries.findIndex(([id]) => id === activeCarId),
    [entries, activeCarId],
  );
  ...
  const { gl } = useThree();
  const { camera } = useThree();
  const gltfMap = useProgressiveLoader(entries, activeIndex, gl, camera);
